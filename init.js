...
.ready(() => {
    ...
    for (var i = 0; i < window.ICO_CONTENT.texts.length; i++) {
        $(`.${data.texts[i]}`).text(data[data.texts[i]]);
    }
    for (var i = 0; i < window.ICO_CONTENT.imgs.length; i++) {
        $(`.${data.imgs[i]}`).attr('src', data[data.imgs[i]]);
    }
